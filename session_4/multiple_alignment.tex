...
\newcommand{\genePair}[3][]{
        ...
        \draw [-,#1] (#2-1.5,#3) -- (#2+0.5,#3);
        \draw [-,line width=2, purple1] (#2-1.0,#3) -- (#2,#3);
        \draw [-,#1] (#2-0.5,#3-0.5) -- (#2+1.5,#3-0.5);
        \draw [-,line width=2, purple3] (#2,#3-0.5) -- (#2+1,#3-0.5);  
}

\newcommand{\enode}{
  node [inner sep=0, outer sep=0]
}
...
%      \foreach \y in {1,2,...,12} \node [font=\small] at (20,\y) {\y};
      ...
      \visible<4->{
        ...
        \sum_{k=1}^n{d_{1,k}} - \sum_{k=1}^n{d_{2,k}}$};
...
\begin{frame}{Neighbor joining (2)}
  The pair of nodes ($f$, $g$) with the lowest $Q$ value are joined through a
  new node $u$. \\
  The distances
  between the new node $u$ and $f$ and $g$ are:
  \begin{equation}
    \begin{aligned}
    ...
    \end{aligned}
  \end{equation}
  $\delta_{f,u}$ and $\delta_{g,u}$ 
  are adjusted such that they are proportional to their
  respective distances to the remaining nodes.
  \blfootnote{Should be: $k \ne f,g$ }
\end{frame}
...
\begin{frame}{ ehh again? }
  ...
  \begin{figure}[ht]
    \begin{tikzpicture}[scale=0.5, every
      ...
      \visible<4->{
        \draw [<->,red] (f) -- (u) node [midway, above right, fill=white, draw=none, rectangle]
        {\tiny $\delta_{f,u}$};

      }
      \visible<4>{
        \draw [<->,red] (g) -- (u) node [midway, fill=white, draw=none, rectangle]
        {\tiny $\delta_{g,u}$};
      }
      \visible<5->{
...
%              { \tiny d_{f,g} };
      ...
      \visible<6->{
       ...
       {\small $( ( d_{f,3} + d_{f,4} + d_f{f,5} + d_{f,6})$}; 
      }
      \visible<6>{
%%        \draw [<->] (f) -- (g);
        \draw [<->] (f) -- (n3);
      ...
      \visible<7->{
       ...
       {\small $-( d_{g,3} + d_{g,4} + d_f{g,5} + d_{g,6}))$}; 
      }
      \visible<7>{
%%        \draw [<->] (f) -- (g);
        \draw [<->] (g) -- (n3);
    ...
    \end{tikzpicture}
  \end{figure}
  \blfootnote{Here $f$ and $g$ are equivalent to 1 and 2}
  \blfootnote{Should be: $k \ne f,g$ }
\end{frame}
...
\begin{frame}[fragile]{Why weight sequences}
  \begin{tikzpicture}[scale=1.0]
    %% \draw [help lines, opacity=1] (0,0) grid (12,10);
    %% \foreach \x in {1,2,...,12} \node [font=\small] at (\x,2) {\x};
    %% \foreach \y in {1,2,...,10} \node [font=\small] at (11,\y) {\y};
    \draw [-] (0,6) \enode (n1) {} -- (1.5,6) \enode (n2) {};
    \draw [-] (n2) -- (2.5,7) \enode (n3) {};
    \draw [-] (n3) -- (2.75,6.5) \enode (n4) {};
    \draw [-] (n3) -- (2.8,7.5) \enode (n5) {};
    \draw [-] (n4) -- (3.2,6.25) \enode (n6) {};
    \draw [-] (n4) -- (3.1,6.75) \enode (n7) {};
    \draw [-] (n5) -- (3.2,7.25) \enode (n8) {};
    \draw [-] (n5) -- (3.4,7.75) node [right, scale=0.5] (n9) {A};
    \draw [-] (n6) -- (3.4,6.0) node [right, scale=0.5] (n10) {G};
    \draw [-] (n6) -- (3.5,6.3) node [right, scale=0.5] (n11) {F};
    \draw [-] (n7) -- (3.4,6.8) node [right, scale=0.5] (n12) {D};
    \draw [-] (n7) -- (3.35,6.6) node [right, scale=0.5] (n13) {E};
    \draw [-] (n8) -- (3.4,7.5) node [right, scale=0.5] (n14) {B};
    \draw [-] (n8) -- (3.4,7.1) node [right, scale=0.5] (n15) {C};
    \draw [-] (n2) -- (3.2, 5.7) \enode (n16) {};
    \draw [-] (n16) -- (3.4,5.75) node [right, scale=0.5] (n17) {H};
    \draw [-] (n16) -- (3.41,5.55) node [right, scale=0.5] (n18) {I};
    \draw [-] (1.25,6) \enode (n19) {} -- (3.4,5.4) node [right, scale=0.5] (n20) {J};
    %% draw the sequence associated with these
    \node (n21) [scale=2,rotate=90] at (3.2,8) {\}};
    \node [above, align=left, scale=0.5] at (n21) {closely related\\species};
    \visible<2->{
      \filldraw [gray] (n3) circle (1pt);
      \node [ above right, text width=13em, scale=0.7] at (6,6) {
        \texttt{A.  GKSWKALTPP}\\
        \texttt{B.  GKSWKSLSPS}\\
        \texttt{C.  GKSWKSLSTS}\\
        \texttt{D.  GKSWKSLSPS}\\
    ...
    \visible<3->{
      \filldraw [black] (n2) circle (1pt);
      \node [ below right, text width=13em, scale=0.7] at (6,5.5) {
        \texttt{H.  PKSWRASSPS}\\
        \texttt{I.  PRSWKSSSPS}\\
      };
    }
    \visible<3>{
      \draw [red] (6.45,6) rectangle (6.625,8.3);
      \draw [red] (6.45,4.9) rectangle (6.625,5.5);
      \draw [<->] (6.53,5.5) -- (6.53,6);
      \node (l1) [below right, scale=0.8] at (1.5,4.5) {
        $ S = \frac{ 7 \times M_{P,G} + 7 \times M_{P,G} }{ 14 } $
      };
    }
    \visible<4->{
      \filldraw [red] (n19) circle (1pt);
      \node [ below right, text width=13em, scale=0.7] at (6,4.5) {
        \texttt{J.  PKSWRALSPS}\\
      };
      \draw [red] (6.45,4.9) rectangle (6.625,8.3);
      \draw [red] (6.45,4.2) rectangle (6.625,4.5);
      \draw [<->] (6.53,4.5) -- (6.53,4.9);
      \node (l1) [below right, scale=0.8] at (1.5,4.5) {
        $ S = \frac{ 7 \times M_{P,G} + 2 \times M_{P,P} }{ 9 } $
      };
      \node [scale=0.7] at (3,3) {$S$ Dominated by $M_{P,G}$};
    }
  \end{tikzpicture}
\end{frame}
...
\begin{frame}{works ok!}
  \includegraphics[width=0.75\textwidth]{images/clustal_alignment}
  \blfootnote{Thompson \emph{et al}, NAR 1994, 22; 4673-4680}
\end{frame}
...
\begin{frame}{Other methods}
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \begin{figure}[ht]
        \includegraphics[width=0.9\textwidth]{images/ebi_MSA_programs}
      \end{figure}
